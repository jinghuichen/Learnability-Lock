...
class conv_iresnet_block(nn.Module):
    def __init__(self, in_shape, int_ch, numTraceSamples=0, numSeriesTerms=0,
        ...
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(in_ch, int_ch, kernel_size=kernel_size1, stride=1, padding=1, bias=False),
                                                  (in_ch, h, w), kernel_size1))
        layers.append(nonlin())
        kernel_size2 = 1 # kernel size for second conv
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(int_ch, int_ch, kernel_size=kernel_size2, padding=0, bias=False),
                                                  (int_ch, h, w), kernel_size2))
        layers.append(nonlin())
        kernel_size3 = 3 # kernel size for third conv
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(int_ch, in_ch, kernel_size=kernel_size3, padding=1, bias=False),
...
#             layers.append(nonlin())
        ...
        in_ch = in_ch * stride**2
        kernel_size1 = 3 # kernel size for first conv
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(in_ch, 32, kernel_size=kernel_size1, stride=1, padding=1, bias=False),
                                                  (in_ch, h, w), kernel_size1))
        layers.append(nonlin())
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(32, 128, kernel_size=kernel_size1, stride=1, padding=1, bias=False),
                                                  (32, h, w), kernel_size1))
        layers.append(nonlin())

        kernel_size2 = 1 # kernel size for second conv

        layers.append(self._wrapper_spectral_norm(nn.Conv2d(128, 256, kernel_size=kernel_size2, padding=0, bias=False),
                                                  (128, h, w), kernel_size2))
        layers.append(nonlin())
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(256, 128, kernel_size=kernel_size2, padding=0, bias=False),
                                                  (256, h, w), kernel_size2))
        layers.append(nonlin())
        kernel_size3 = 3 # kernel size for third conv

        layers.append(self._wrapper_spectral_norm(nn.Conv2d(128, 32, kernel_size=kernel_size3, padding=1, bias=False),
                                                  (128, h, w), kernel_size3))
        layers.append(nonlin())
        layers.append(self._wrapper_spectral_norm(nn.Conv2d(32, in_ch, kernel_size=kernel_size3, padding=1, bias=False),
                                                  (32, h, w), kernel_size3))
        self.bottleneck_block = nn.Sequential(*layers)
        self.layers = layers
        if actnorm:
...
class Conv_iResNet(nn.Module):
    def __init__(self, mid_planes=8, in_planes=3, in_shape = 32, num_classes=10, num_layers=1, epsilon=0.032):
        ...
        for i in range(num_layers):
            self.blocklist.append(conv_iresnet_block3([in_planes, in_shape, in_shape], mid_planes, stride=1, actnorm=False, nonlin='relu', epsilon=self.epsilon/num_layers))
